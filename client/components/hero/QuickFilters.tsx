      ...
      <div className="w-full px-4 mt-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {QUICK_FILTERS.slice(0, 5).map((filter, i) => (
            <button suppressHydrationWarning key={i} className="px-3 py-1.5 rounded-full text-[10px] font-bold text-zinc-300 bg-white/5 border border-white/5 hover:bg-white/10 hover:text-white transition-colors">
              {filter}
            </button>
          ))}
        </div>
