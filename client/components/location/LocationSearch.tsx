    ...
    <div ref={containerRef} className="w-full relative">
      {!selectedLocation ? (
        ...
        <div className="w-full relative">
          <div className="w-full flex items-center h-12 px-4 rounded-2xl bg-white shadow-sm border border-gray-200 focus-within:border-brand-purple focus-within:ring-1 focus-within:ring-brand-purple/50 focus-within:shadow-[0_0_12px_rgba(139,92,246,0.15)] transition-all gap-3">
            ...
            <input suppressHydrationWarning
              ...
              className="bg-transparent border-none outline-none text-sm text-gray-900 placeholder-gray-400 w-full font-semibold"
            />
            {inputValue && (
              <button suppressHydrationWarning
                type="button"
                onClick={handleClear}
                className="p-1 rounded-md text-gray-500 hover:text-white hover:bg-white/5 transition-all shrink-0 cursor-pointer"
              >
          ...
          {isOpen && inputValue.trim() !== "" && (
            ...
            >
              {loading && (
                <div className="flex items-center gap-2 px-3 py-3 text-gray-500 text-xs font-semibold font-sans">
              ...
              {!loading && !error && suggestions.length === 0 && (
                <div className="px-3 py-3 text-gray-500 text-xs font-semibold font-sans">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col leading-tight min-w-0">
                      <span className="text-xs font-bold text-gray-900 truncate">{primary}</span>
                      {secondary && <span className="text-[10px] text-gray-500 truncate mt-0.5">{secondary}</span>}
                    </div>
                  </button>
                );
              })}
        ...
        <div className="w-full h-12 bg-brand-purple/5 border border-brand-purple/20 px-3.5 rounded-2xl flex items-center justify-between gap-3 text-left shadow-lg backdrop-blur-md">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <MapPin className="h-4.5 w-4.5 text-brand-purple shrink-0 animate-pulse" />
            <span className="text-xs font-black text-gray-900 truncate flex-1 leading-none">
          ...
          <button suppressHydrationWarning
            type="button"
            onClick={handleClear}
            className="h-7 w-7 rounded-lg border border-brand-purple/20 bg-white text-gray-500 hover:text-rose-500 hover:bg-rose-50 hover:border-rose-200 transition-all shrink-0 cursor-pointer flex items-center justify-center"
