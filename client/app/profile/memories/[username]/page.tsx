...
export default function UserMemoriesPage() {
  ...
  return (
    <div className="min-h-screen bg-mesh-premium text-gray-900 flex flex-col items-center py-6 px-4 md:px-8 relative overflow-x-hidden selection:bg-cyan-100/30 selection:text-gray-900">
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div 
            ...
            className="fixed top-6 bg-white border-gray-200 border border-brand-cyan/30 px-5 py-3 rounded-2xl text-xs font-mono font-bold text-brand-cyan shadow-[0_0_20px_rgba(6,182,212,0.15)] z-50 flex items-center gap-2.5"
          >
            <span className="h-2 w-2 rounded-full bg-cyan-100 animate-pulse" />
            {toastMessage}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-6xl w-full flex flex-col gap-8 mt-12 sm:mt-0">
        ...
        <header className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-gray-200 pb-6">
          <div className="hidden sm:flex items-center gap-4 text-left">
            <div className="flex flex-col text-left">
              ...
              <p className="text-xs text-gray-500 font-mono mt-0.5">Index of adventures and journals</p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between sm:justify-end gap-3.5 w-full sm:w-auto">
            {dbProfile && (
              <div 
                ...
                className="flex items-center gap-3 bg-white border border-gray-200 px-3 py-1.5 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors"
              >
                <div className="h-7 w-7 rounded-full bg-gradient-to-tr from-brand-indigo to-brand-purple p-0.5 shadow-md shrink-0">
                  <div className="h-full w-full rounded-full bg-white border-gray-200 overflow-hidden flex items-center justify-center font-bold text-[10px]">
                    {dbProfile.avatarUrl ? (
                ...
                <div className="flex flex-col text-left">
                  <span className="text-[10px] font-black text-gray-900 leading-none">{dbProfile.displayName}</span>
                  <span className="text-[8px] font-mono text-gray-500 mt-0.5">@{dbProfile.username}</span>
                </div>
              </div>
            )}
            ...
            {isOwner && (
              <button
                ...
                className="h-9 px-4 rounded-xl border border-brand-cyan/20 bg-cyan-100/10 hover:bg-cyan-100/25 text-xs font-bold uppercase tracking-wider text-brand-cyan hover:text-gray-900 flex items-center gap-1.5 transition-all cursor-pointer shadow-md shrink-0"
              >
        ...
        {memories.length === 0 && (
          <div className="py-24 text-center flex flex-col items-center justify-center gap-3">
            <Compass className="h-10 w-10 text-gray-400 animate-pulse" />
            <p className="text-sm font-mono text-gray-500 uppercase tracking-widest">No Memories Logged</p>
            <p className="text-xs text-gray-400 max-w-xs">There are no memory coordinates published under this profile.</p>
          </div>
        )}
          ...
          {memories.map((memory) => {
            ...
            return (
              <motion.article 
                ...
                className="bg-white border border-gray-200 rounded-3xl overflow-hidden hover:border-gray-300 hover:shadow-[0_15px_30px_rgba(0,0,0,0.4)] transition-all duration-300 flex flex-col h-full group/card"
              >
                {/* Card Image Area */}
                <div className="w-full h-48 overflow-hidden relative select-none bg-white border-gray-200">
                  <img 
                    src={memoryImage} 
                    alt={memory.title} 
                  ...
                  <span className="absolute top-4 left-4 text-[9px] font-black text-zinc-950 bg-cyan-100 border border-brand-cyan/20 px-3 py-1 rounded-full uppercase tracking-wider shadow-lg">
                    {memory.category.toUpperCase()}
                  </span>
                  
                  <div className="absolute bottom-4 left-4 flex items-center gap-1.5 text-gray-700 text-[10px] font-mono font-bold bg-gray-50/70 border border-gray-200 backdrop-blur-sm px-2.5 py-1 rounded-lg">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between gap-5">
                  <div className="text-left">
                    <h3 className="text-sm font-black text-gray-900 group-hover/card:text-brand-cyan transition-colors leading-tight">
                      {memory.title}
                    </h3>
                    <p className="text-xs text-gray-500 font-semibold leading-relaxed mt-2.5 line-clamp-4">
                      {memory.content}
                    </p>
                  </div>

                  {/* Social interactions section */}
                  <div className="border-t border-gray-200 pt-3.5 flex items-center justify-between text-gray-500 text-xs font-mono">
                    <div className="flex items-center gap-4 w-full">
                      ...
                      <button 
                        ...
                        className="flex items-center gap-1.5 hover:text-gray-900 transition-colors cursor-pointer select-none group/btn"
        ...
        {memories.length > 0 && (
          <footer className="text-center py-12 border-t border-gray-200 mt-6">
        ...
        {activeCommentMemory && (
          <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm z-50 flex items-center justify-end">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-md h-full bg-white border-gray-200 border-l border-gray-200 flex flex-col justify-between shadow-2xl z-10"
            >
              {/* Header */}
              <div className="p-5 border-b border-gray-200 flex items-center justify-between">
                <div className="flex flex-col text-left">
                  <h3 className="text-sm font-black text-gray-900 uppercase tracking-wider">Comments Thread</h3>
                ...
                <button
                  onClick={() => setActiveCommentMemory(null)}
                  className="h-8 w-8 rounded-lg bg-gray-100 hover:bg-zinc-700 text-gray-500 hover:text-gray-900 flex items-center justify-center transition-all cursor-pointer"
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {isCommentsLoading ? (
                  ...
                  activeComments.map((comment: any) => (
                    <div 
                      key={comment.id}
                      className="flex gap-3 items-start text-xs text-left bg-white p-3.5 rounded-2xl border border-gray-200"
                    >
                      <div className="h-7 w-7 rounded-full bg-gray-100 border border-white/10 flex items-center justify-center font-bold text-[10px] shrink-0 text-gray-700 select-none">
                        {comment.user.displayName.trim().charAt(0).toUpperCase()}
                      </div>
                      <div className="flex flex-col gap-1 flex-1">
                        <div className="flex items-center justify-between">
                          <span className="font-black text-gray-900">{comment.user.displayName}</span>
              ...
              <form 
                ...
                className="p-4 bg-gray-50 border-t border-gray-200 flex gap-2 items-center"
              >
                <input 
                  type="text"
                  ...
                  className="flex-1 h-10 px-4 rounded-xl bg-white border-gray-200 border border-gray-200 text-xs text-gray-900 placeholder-zinc-500 focus:outline-none focus:border-brand-cyan/40 transition-colors font-semibold"
                />
                <button
                  type="submit"
                  disabled={!newCommentText.trim() || addCommentMutation.isPending}
                  className="h-10 w-10 rounded-xl bg-cyan-100 hover:bg-cyan-500 text-zinc-950 flex items-center justify-center transition-all disabled:opacity-30 disabled:pointer-events-none cursor-pointer shadow-[0_0_15px_rgba(6,182,212,0.2)]"
