...
export default function UserMemoriesPage() {
  ...
  // Post memory modal states
  const [isPostModalOpen, setIsPostModalOpen] = useState(false);
  const [postMemoryTitle, setPostMemoryTitle] = useState("");
  const [postMemoryTag, setPostMemoryTag] = useState("");
  const [postMemoryText, setPostMemoryText] = useState("");
  const [postMemoryLocation, setPostMemoryLocation] = useState<{formatted_address: string; latitude: number; longitude: number;} | null>(null);
  const [postMemoryImages, setPostMemoryImages] = useState<string[]>([]);
        ...
        {isPostModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="bg-zinc-900 border border-white/12 rounded-3xl w-full max-w-md p-6 relative z-10 shadow-2xl flex flex-col gap-4 text-left max-h-[90vh] overflow-y-auto"
            >
              {/* Close Button */}
              <button
                onClick={() => setIsPostModalOpen(false)}
                className="absolute top-4 right-4 h-8 w-8 rounded-lg bg-white/5 border border-white/5 text-zinc-400 hover:text-white flex items-center justify-center cursor-pointer transition-colors"
              >
                <X className="h-4 w-4" />
              </button>
              ...
              <div>
                <h2 className="text-base font-black text-white flex items-center gap-2">
                  <Compass className="h-5 w-5 text-brand-cyan" />
                  Post Adventure Memory
                </h2>
              </div>

              {validationError && (
                <div className="p-3 bg-rose-500/10 border border-rose-500/20 text-rose-500 rounded-xl text-[11px] font-mono font-bold">
                  ⚠️ {validationError}
                </div>
              )}
              ...
              <form onSubmit={(e) => {
                e.preventDefault();
                if (!postMemoryTitle.trim() || !postMemoryTag.trim() || !postMemoryText.trim() || !postMemoryLocation) {
                  setValidationError("All fields are required. Please fill in coordinates, tags, details and location.");
                  return;
                }
                if (postMemoryImages.length < 1) {
                  setValidationError("At least 1 image is required to post memories.");
                  return;
                }
                if (postMemoryImages.length > 4) {
                  setValidationError("You can only upload a maximum of 4 images for a single post.");
                  return;
                }
                
                // Add memory to list
                const newMemoryId = `mem-user-${Date.now()}`;
                const newMem: Memory = {
                  id: newMemoryId,
                  title: postMemoryTitle,
                  text: postMemoryText,
                  tag: postMemoryTag.toUpperCase(),
                  location: postMemoryLocation.formatted_address,
                  image: postMemoryImages[0], // first image as main card image
                  likes: 0,
                  commentsCount: 0,
                  liked: false,
                  saved: false,
                ...
                setPostMemoryTag("");
                setPostMemoryText("");
                setPostMemoryLocation(null);
                setPostMemoryImages([]);
                setValidationError(null);
                triggerToast("Adventure memory successfully synchronized!");
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input
                    type="text"
                    required
                    value={postMemoryTitle}
                    onChange={(e) => setPostMemoryTitle(e.target.value)}
                    placeholder="e.g. Milestone Earned at Varkala Cliffside"
                ...
                {/* Tag */}
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-mono font-bold uppercase tracking-wider text-zinc-500">Category Tag</label>
                  <input
                    type="text"
                    required
                    value={postMemoryTag}
                    onChange={(e) => setPostMemoryTag(e.target.value)}
                    placeholder="e.g. Milestone Earned"
                    className="h-10 px-3.5 rounded-xl bg-zinc-950/40 border border-white/10 text-xs text-white placeholder-zinc-600 focus:border-brand-cyan focus:outline-none transition-colors font-semibold"
                  />
                </div>

                {/* Geoapify Autocomplete Location Field */}
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-mono font-bold uppercase tracking-wider text-zinc-500">Coordinates (Location)</label>
                  <LocationSearch
                    onSelect={setPostMemoryLocation}
                    selectedLocation={postMemoryLocation}
                  />
                </div>

                {/* Text Description */}
                <div className="flex flex-col gap-1">
                  <label className="text-[9px] font-mono font-bold uppercase tracking-wider text-zinc-500">Adventure Details</label>
                  <textarea
                    required
                    rows={3}
                    value={postMemoryText}
                    onChange={(e) => setPostMemoryText(e.target.value)}
                    placeholder="Write details of your experience..."
                    className="p-3.5 rounded-xl bg-zinc-950/40 border border-white/10 text-xs text-white placeholder-zinc-600 focus:border-brand-cyan focus:outline-none transition-colors font-semibold resize-none"
                  />
                </div>

                {/* Images Upload */}
                <div className="flex flex-col gap-2">
                  <label className="text-[9px] font-mono font-bold uppercase tracking-wider text-zinc-500">Snapshots (Max. 4 Images)</label>
                  ...
                  <input
                    ...
                    multiple
                    className="hidden"
                    id="post-memory-images-input"
                    onChange={(e) => {
                      const files = Array.from(e.target.files || []);
                      const hasVideo = files.some(file => file.type.startsWith("video/"));
                      if (hasVideo) {
                        setValidationError("Videos are not allowed. Please upload images only.");
                        return;
                      }

                      const totalCount = postMemoryImages.length + files.length;
                      if (totalCount > 4) {
                        setValidationError("You can only upload a maximum of 4 images for a single post.");
                        return;
                      }
                      ...
                      Promise.all(readPromises).then(base64Images => {
                        setPostMemoryImages(prev => [...prev, ...base64Images]);
                        setValidationError(null);
                      });
                    }}
                  />
                  ...
                  <label
                    htmlFor="post-memory-images-input"
                    className="h-10 px-4 rounded-xl border border-dashed border-white/20 hover:border-brand-cyan/40 bg-white/[0.01] hover:bg-white/[0.03] text-xs font-bold text-zinc-400 hover:text-zinc-200 transition-all flex items-center justify-center gap-2 cursor-pointer"
                  >
                    <Plus className="h-4 w-4 text-zinc-500" />
                    Select files (Max. 4 images)
                  </label>

                  {/* Previews */}
                  {postMemoryImages.length > 0 && (
                    <div className="grid grid-cols-4 gap-2 mt-1">
                      {postMemoryImages.map((img, idx) => (
                        <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-white/10 group">
                          <img src={img} alt="Preview" className="w-full h-full object-cover" />
                          <button
                            type="button"
                            onClick={() => setPostMemoryImages(prev => prev.filter((_, i) => i !== idx))}
                            className="absolute top-1 right-1 h-5 w-5 rounded-full bg-zinc-950/80 border border-white/10 text-zinc-400 hover:text-white flex items-center justify-center cursor-pointer transition-colors"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </div>
                ...
                <button
                  type="submit"
                  className="mt-2 h-11 w-full rounded-xl bg-gradient-to-r from-brand-indigo to-brand-purple text-white font-bold text-xs uppercase tracking-wider hover:brightness-110 active:scale-98 transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg"
                >
                  Publish Coordinates
                </button>
              </form>
