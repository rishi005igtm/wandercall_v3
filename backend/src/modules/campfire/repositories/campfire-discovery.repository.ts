...
import {
  CampfireCursorPaginationDto,
  CampfireSortField,
  CampfireDiscoveryResponse,
} from '../dto/campfire-discovery.dto';
...
export class CampfireDiscoveryRepository {
  ...
  async findDiscoverable(
    query: CampfireCursorPaginationDto,
  ): Promise<CampfireDiscoveryResponse> {
    ...
    qb.leftJoin(
      'users_profile',
      'userProfile',
      '"userProfile"."userId" = campfire."hostId"',
    );
    qb.leftJoin(
      'users_auth',
      'userAuth',
      '"userAuth"."id" = campfire."hostId"',
    );
    qb.addSelect([
    ...
    const typedRaw = raw as Array<{
      campfire_id: string;
      userProfile_displayName?: string;
      userAuth_displayName?: string;
      userProfile_avatarUrl?: string;
      userProfile_username?: string;
    }>;
    items.forEach((item) => {
      const rawRow = typedRaw.find((r) => r.campfire_id === item.id);
      if (rawRow) {
        item.hostName =
          rawRow.userProfile_displayName ||
          rawRow.userAuth_displayName ||
          'Wanderer';
  ...
  private applyFilters(
    qb: SelectQueryBuilder<CampfireEntity>,
    query: CampfireCursorPaginationDto,
  ) {
    ...
    if (query.category) {
      qb.andWhere('campfire.category = :category', {
    ...
    if (query.visibility) {
      qb.andWhere('campfire.visibility = :visibility', {
    ...
    if (query.status) {
      if (
        (query.status as string) === 'LIVE' ||
        (query.status as string) === 'ACTIVE'
      ) {
        qb.andWhere('campfire.status IN (:...statuses)', {
          statuses: ['LIVE', 'ACTIVE'],
        });
      } else {
    ...
    if (query.communityId) {
      qb.andWhere('campfire.communityId = :communityId', {
  ...
  private applyCursorAndSort(
    ...
    switch (sort) {
      case CampfireSortField.OLDEST:
        ...
        if (cursor) {
          qb.andWhere('campfire.createdAt > :cursor', {
      ...
      case CampfireSortField.RECENTLY_STARTED:
        ...
        if (cursor) {
          qb.andWhere('campfire.startedAt < :cursor', {
            cursor: new Date(cursor),
          });
        }
        break;
  ...
  private generateNextCursor(
    lastItem: CampfireEntity,
    sort?: CampfireSortField,
  ): string {
    switch (sort) {
      case CampfireSortField.OLDEST:
      case CampfireSortField.POPULARITY:
