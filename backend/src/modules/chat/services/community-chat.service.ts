...
import { MessageEntity, MessageType, MessageStatus } from '../entities/message.entity';
...
export class CommunityChatService {
  ...
  async sendMessage(senderId: string, communityId: string, dto: SendMessageDto): Promise<SendMessageAckDto> {
    this.enforceRateLimit(senderId);

    const member = await this.communityMemberRepo.findByUserAndCommunity(senderId, communityId);
    if (!member || member.status !== CommunityMemberStatus.ACTIVE) {
      throw new ForbiddenException('You must be an active member of this community to send messages.');
    }
    if (member.isMuted) {
      throw new ForbiddenException('You are currently muted in this community.');
    }
    ...
    const { message: sentMessage, isDuplicate } = await this.messageService.persistMessage(
      senderId,
      conversationId,
      dto,
    ...
    if (!isDuplicate) {
      ...
      `, [conversationId]).catch(err => {
        this.logger.error(`Failed to enforce message cap for community conversation ${conversationId}`, err);
      });

      this.chatEventDispatcher.dispatchCommunityMessage(communityId, sentMessage);
    }
  ...
  async getMessages(userId: string, communityId: string, conversationId: string, cursor?: Date, limit = 50) {
    const member = await this.communityMemberRepo.findByUserAndCommunity(userId, communityId);
    if (!member || member.status !== CommunityMemberStatus.ACTIVE) {
      throw new ForbiddenException('Not an active member of this community.');
    }

    return this.messageRepository.paginate(conversationId, limit, cursor?.toISOString());
  }
  ...
  async getDefaultConversation(communityId: string): Promise<ConversationEntity> {
    let conv = await this.conversationRepository.findDefaultCommunityConversation(communityId);
    if (!conv) {
      this.logger.warn(`Default conversation not found for community ${communityId}. Auto-provisioning...`);
      conv = await this.conversationRepository.createCommunityConversation(communityId);
    }
    return conv;
  }
}
