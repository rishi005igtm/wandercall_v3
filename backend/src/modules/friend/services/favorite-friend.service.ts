...
export class FavoriteFriendService {
  ...
  async addFavorite(userId: string, friendId: string): Promise<FavoriteFriendEntity> {
    if (userId === friendId) {
      throw new BadRequestException("You cannot favorite yourself.");
    }

    // Verify mutual friendship
    const rel = await this.relationshipService.resolveRelationship(userId, friendId);
    if (!rel.isFriend) {
    ...
    if (currentFavs.length >= this.MAX_FAVORITES) {
      throw new BadRequestException(`You cannot exceed the maximum limit of ${this.MAX_FAVORITES} favorites.`);
    }
  ...
  async reorderFavorites(userId: string, orderedFriendIds: string[]): Promise<void> {
