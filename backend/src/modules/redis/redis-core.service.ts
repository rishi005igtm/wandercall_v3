...
export class RedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (redisUrl) {
      ...
      this.tcpClient.on('error', (err) => this.logger.error('Redis TCP Client Error:', err));
      this.tcpSubscriber.on('error', (err) => this.logger.error('Redis TCP Subscriber Error:', err));
    } else {
   ...
   */
  generateKey(tenantId: string, entity: string, entityId: string, suffix?: string): string {
