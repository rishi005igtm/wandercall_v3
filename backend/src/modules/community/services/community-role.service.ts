...
export class CommunityRoleService {
  ...
  async createCustomRole(params: {
    ...
    if (existing) {
      throw new ConflictException(
        `Role with name ${formattedName} already exists`,
      );
    }
  ...
  async updateRole(
    ...
    if (!role.editable && role.systemRole) {
      ...
      if (params.priority !== undefined && params.priority !== role.priority) {
        throw new ForbiddenException(
          `Cannot change priority of system role ${role.name}`,
        );
      }
    }
    ...
    if (
      ...
      throw new BadRequestException(
        'Priority <= 1 is strictly reserved for OWNER',
      );
    }

    if (params.displayName !== undefined) role.displayName = params.displayName;
    if (params.displayColor !== undefined)
      role.displayColor = params.displayColor;
  ...
  async deleteRole(roleId: string): Promise<void> {
    ...
    if (role.protected || role.systemRole) {
      throw new ForbiddenException(
        `System or protected role '${role.name}' cannot be deleted`,
      );
    }
    ...
    if (defaultMemberRole) {
      const affectedMembers = await this.memberRepo.find({
  ...
  async assignRole(
    ...
    if (targetMember.isOwner) {
      throw new BadRequestException(
        'Cannot change the role of the community owner. Use transfer ownership.',
      );
    }
    ...
    if (!roleToAssign) {
      ...
      if (
        !roleToAssign &&
        (normalized === 'COMMUNITY_GUIDE' || normalized === 'GUIDE')
      ) {
          ...
          allRoles.find(
            (r) =>
              r.displayName?.toLowerCase() ===
                roleIdOrName.trim().toLowerCase() || r.name === normalized,
          ) || null;
      }
    }
    if (!roleToAssign) {
    ...
    const targetCurrentRole = targetMember.roleId
    ...
    if (
      !actorMember.isOwner &&
      (actorRole?.priority ?? 1000) >= roleToAssign.priority
    ) {
      throw new ForbiddenException(
        `You cannot assign a role equal to or higher than your own rank (${roleToAssign.displayName || roleToAssign.name})`,
      );
    }

    const oldRoleName =
      targetCurrentRole?.displayName || targetCurrentRole?.name || 'MEMBER';
    ...
    this.eventDispatcher.dispatchRoleChanged(
      id,
      targetUserId,
      roleToAssign.id,
      actorId,
