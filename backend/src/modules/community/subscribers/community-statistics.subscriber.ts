  ...
  onModuleInit() {
    ...
    this.dispatcher.on(CommunityEvents.JOINED, async (payload: { communityId: string; userId: string }) => {
      try {
        ...
        this.logger.error(`Error handling JOINED event for stats: ${err.message}`, err.stack);
      }
    });

    this.dispatcher.on(CommunityEvents.LEFT, async (payload: { communityId: string; userId: string }) => {
      try {
        await this.handleMemberLeft(payload.communityId);
      } catch (err: any) {
        this.logger.error(`Error handling LEFT event for stats: ${err.message}`, err.stack);
      }
    });

    this.dispatcher.on(CommunityEvents.MEMBER_KICKED, async (payload: { communityId: string; userId: string }) => {
      try {
        await this.handleMemberLeft(payload.communityId);
      } catch (err: any) {
        this.logger.error(`Error handling MEMBER_KICKED event for stats: ${err.message}`, err.stack);
      }
    });

    this.dispatcher.on(CommunityEvents.MEMBER_BANNED, async (payload: { communityId: string; userId: string }) => {
      try {
        await this.handleMemberLeft(payload.communityId);
      } catch (err: any) {
        this.logger.error(`Error handling MEMBER_BANNED event for stats: ${err.message}`, err.stack);
      }
    });
  }
