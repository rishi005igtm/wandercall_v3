...
export class DatabaseInitializerService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        await this.dataSource.query(`DROP TABLE IF EXISTS "feed_impressions" CASCADE`);
  ...
  private async runChatIntegrityMigration(): Promise<void> {
    ...
    const duplicateGroups = await this.dataSource.query(`
      SELECT
    ...
    const allConvsByKey = await this.dataSource.query(`
      SELECT
        c.id,
        c."createdAt",
